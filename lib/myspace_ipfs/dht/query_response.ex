defmodule MyspaceIPFS.DhtQueryResponse do
  @moduledoc false
  require Logger
  import MyspaceIPFS.Utils, only: [filter_empties: 1]
  ...
  @spec new(map) :: MyspaceIPFS.Dht.query_response()
  ...
  @spec new(list) :: list(MyspaceIPFS.Dht.query_response())
  ...
  @spec handle_responses(map) :: list(MyspaceIPFS.Dht.query_response_addrs()) | []
  defp handle_responses(attrs) when is_map(attrs) do
    attrs = Map.get(attrs, "Responses", nil)

    case attrs do
      nil -> nil
      _ -> Enum.map(attrs, &MyspaceIPFS.DhtQueryResponseAddrs.new/1)
    end
  end
end
