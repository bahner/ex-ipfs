defmodule ExIPFS.Files do
  @moduledoc """
  ExIPFS.Files is where the files commands of the IPFS API reside.
  """
  import ExIPFS.Api
  import ExIPFS.Utils
  ...
  """
  @type entries :: %ExIPFS.FilesEntries{
          entries: [entry()]
        }
  ...
  @spec cp(Path.t(), Path.t(), list) :: :ok | ExIPFS.Api.error_response()
  ...
  """
  # FIXME: verify return type
  @spec chcid(Path.t(), list) :: {:ok, any} | ExIPFS.Api.error_response()
  ...
  """
  # FIXME: verify return type
  @spec flush() :: {:ok, any} | ExIPFS.Api.error_response()
  def flush() do
    post_query("/files/flush")
    |> okify()
  end

  @spec flush(Path.t()) :: {:ok, any} | ExIPFS.Api.error_response()
  def flush(path) do
  ...
  ```
  """
  @spec ls(Path.t(), list) :: {:ok, list} | ExIPFS.Api.error_response()
  def ls(path, opts \\ []) do
    ls!(path, opts)
    |> okify()
  end
  ...
  @spec mkdir(Path.t(), list) :: :ok | ExIPFS.Api.error_response()
  ...
  """
  @spec mv(Path.t(), Path.t()) :: :ok | ExIPFS.Api.error_response()
  ...
  ```
  """
  @spec read!(Path.t(), list) :: any | ExIPFS.Api.error_response()
  def read!(path, opts \\ []) do
    post_query("/files/read?arg=" <> URI.encode(path), query: opts)
  end

  @spec read(Path.t(), list) :: {:ok, any} | ExIPFS.Api.error_response()
  def read(path, opts \\ []) do
    read!(path, opts)
    |> okify()
  end
  ...
  ```
  """
  @spec stat(Path.t(), list) :: {:ok, any} | ExIPFS.Api.error_response()
  def stat(path, opts \\ []) do
    post_query("/files/stat?arg=" <> URI.encode(path), query: opts)
    |> ExIPFS.FilesStat.new()
    |> okify()
  end
  ...
  ```
  """
  @spec write(binary(), Path.t(), list) :: :ok | ExIPFS.Api.error_response()
