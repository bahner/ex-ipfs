defmodule ExIPFS.PubSub do
  @moduledoc """
  ExIPFS.PubSub is where the pubsub commands of the IPFS API reside.
  """
  import ExIPFS.Api
  import ExIPFS.Utils
  ...
  ## Usage
  ```
  ExIPFS.PubSub.sub("mymessage", "mytopic")
  ```
  ...
  Messages are sent to the process as a tuple of `{:ex_ipfs_pubsub_channel_message, message}`.
  ...
  def sub(pid, topic) do
    ExIPFS.PubSub.Channel.new!(pid, topic)
    |> ExIPFS.PubSub.Channel.start_link()
  end
  ...
  def get_pubsub_channel_message() do
    receive do
      {:ex_ipfs_pubsub_channel_message, message} -> message
    end
  end
